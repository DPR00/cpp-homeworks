...
namespace ipb {
    ...
    vector<Mat> BowDictionary::descriptors() const { return in_descriptors_; }
    ...
    void BowDictionary::set_params(int max_iter,
                                   int dict_size,
                                   vector<Mat> &descriptors) {
        set_descriptors(descriptors);
        set_size(dict_size);
        set_max_iterations(max_iter);
        vocabulary_ = ipb::kMeans(in_descriptors_, dict_size, max_iter);
    }

    void BowDictionary::set_max_iterations(int iters) { max_iter_ = iters; }
